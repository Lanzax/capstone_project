...
const Recipe = () => {
  ...
  const addLocalRecipe = (i) => {
    const newRecipe = {
      "categoria": data[i].categoria,
      "nome": data[i].nome,
      "ingredienti": [data[i].ingredienti],
      "immagine": data[i].immagine,
      "descrizione_piccola": data[i].descrizione_piccola,
      "descrizione_grande": data[i].descrizione_grande,
    };
    setRecipes([...recipes, newRecipe]);
    setclick(true);
  };
  const removeLocalRecipe = (i) => {
  ...
  return (
    <>
      ...
      {data.slice(iStart, iFinish).map((recipe,i) => {
          let counter=i
        return (
            ...
            <div className="recipeCard d-flex align-content-center w-75 ">
              <div className="recipeImg">
                <img src={recipe.immagine} className="" alt="" />
                {
                  (click === false ? (
                    <AiFillHeart
                      onClick={() => addLocalRecipe(counter)}
                      className="fs-2 like icon"
                    />
                  ) : (
                    <AiFillHeart
                      onClick={() => removeLocalRecipe(counter)}
